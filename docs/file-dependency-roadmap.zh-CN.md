...
项目面向 `net10.0` 和 Windows x64。从 NuGet 1.0.1 开始，传递构建文件会自动把未指定平台或使用 `AnyCPU` 的消费项目调整为 `x64`；当前发布版本为 1.0.7。
...
| 文件 | 主要职责 | 依赖 / 调用 | 被谁使用 |
|---|---|---|---|
| `RfcClient.csproj` | 定义 `net10.0`/x64 类库、SAP NCo DLL 引用、依赖和 NuGet 1.0.7 打包布局 | `libs/*.dll`、`Microsoft.Extensions.*`、`buildTransitive/*` | `RfcClient.sln`、构建工具 |
